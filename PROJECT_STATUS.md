...
| M5.2 | 主 Agent 委派与父子 Run | 已完成 | 专项通过；当前全量 46/46 | 待执行 | **待人工验收** |
| M4.1 | ChatGPT 式多轮 Web 与 Subagent 卡片 | 已完成 | 39/39 + HTTP/SSE 通过 | 待执行 | **待人工验收** |
| M4.2 | 意图路由与对话/编码双模式 | 已完成 | 46/46 + 路由/HTTP/SSE 通过 | 待执行 | **待人工验收** |
...
- [x] Web 输入区提供 `自动 / 对话 / 编码` 三种请求模式；
- [x] 自动模式把问候、闲聊路由到零工具 ChatAgent，不再扫描仓库；
- [x] 自动模式把仓库定位、解释类问题路由到只读 Agent，只开放 list/search/read；
- [x] 仅在明确要求修改时进入完整 CodingAgent，保留计划、Subagent、Patch、测试和完成门禁；
- [x] 每轮持久化请求模式和实际意图，刷新或服务重启后仍可恢复并显示；
- [x] SQLite 自动迁移 M4.1 的既有会话库，不丢失历史会话；
- [x] Web 显示路由徽标，流式答案只呈现最后一次模型回复，不再拼接中间工具轮文本；
- [x] DeepSeek 零工具请求不发送空 `tools` 字段；
- [x] 新建对话自动恢复安全的 `自动` 模式。

自动验收：2026-09-01 全量 `46/46` 通过。测试覆盖三类自动路由、手动模式覆盖、零工具问候、只读仓库问答、旧数据库迁移、持久化和 HTTP/SSE。

人工验收：待用户运行 `.\start.cmd webdemo`，依次验证“你好”零工具、“项目入口在哪里”只读工具，以及明确修复任务进入完整编码流程。
...
自动验收：M5.2 完成时全量 `37/37` 通过；加入 M4.2 后，当前项目基线为 `46/46`。尚未执行真实 DeepSeek 父子 Coding Demo，避免在未告知用户的情况下继续产生两层模型调用成本。
...
| 2026-09-01 | 完成 M4.2 意图路由、零工具对话、只读问答和手动对话/编码模式 | 离线测试 46/46；浏览器人工验收待执行 |
