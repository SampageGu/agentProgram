...
> 当前阶段：**M5.2 主 Agent 委派等待人工验收**  
> 下一阶段：**M5.3 父子 Replay 与 Web Subagent 卡片**

本文是项目进度的单一事实来源。`Agent项目计划表.md` 描述长期路线，本文件记录现在实际做到了哪里、哪些验收已经通过以及下一步做什么。
...
| M5.2 | 主 Agent 委派与父子 Run | 已完成 | 37/37 通过 | 待执行 | **待人工验收** |
...
详细判定规则见 [docs/验收手册.md](docs/验收手册.md)。
...
人工验收：待用户使用真实 DeepSeek Key 执行，步骤见 [docs/验收手册.md](docs/验收手册.md)。
...
人工验收：待用户运行 `.\start.cmd demo` 和 `.\start.cmd eval1`。正式通过标准见 [docs/验收手册.md](docs/验收手册.md)。
...
自动验收：2026-09-01 全量 `37/37` 通过。尚未执行真实 DeepSeek 父子 Coding Demo，避免在未告知用户的情况下继续产生两层模型调用成本。
...
- M5.2 已能真正委派，但 Replay 仍需分别传入父/child Run ID，M4 页面也尚未显示 Subagent 卡片；这两项属于 M5.3；
